...
def main():
    ...
    hyperparameter_ranges = {
        'n_estimators': randint(750, 1400),  # Larger range for robustness, common in financial modeling #first 500
        'max_features': ["sqrt"],  # Focus on square root of features for generalization
        'max_depth': randint(7, 10),  # Slightly increased depth range to capture more complexity
        ...
        'min_impurity_decrease': uniform(0.000, 0.002), # first 0.005
        'criterion': ['entropy']  # Expanded criterion options for impurity calculation # only entropy first
}

    # with open('output/rf/rf_best_hyperparameters.json', 'r') as f:
    #     params = json.load(f)
